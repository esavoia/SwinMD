...
//    psize = MPI::COMM_WORLD.Get_size();
    ...
    if(prank ==0) 
    {
        ...
        *ofPotential<<"           LJEn"<<"   realEn"<<" longEn"<<"   molCor"<<\
...
void LFIntegrator :: initialise()
{
    ...
    if (has_constr_force) 
    {
        ...
        if ((rOld==NULL)||(rNew==NULL)||(moving==NULL)||(moved==NULL)||(bondLength==NULL))
            ERRORMSG("memory allocation error for LFIntegrator");
        for (int nm = 0; nm < numMols; nm++)
        {
            ...
            if((rOld[nm]==NULL)||(rNew[nm]==NULL)||(moving[nm]==NULL)||(moved[nm]==NULL)||(bondLength[nm]==NULL))
                ERRORMSG("memory allocation error for LFIntegrator");           
        }
        ...
        for (int nm = 0; nm < numMols; nm++)
        {
            ...
            if (!strcmp(myMols[nm].molName, "WAT"))
            { 
	    ...
	    if (!strcmp(thermotype, "NoseHoover")){
	     ...
	     if (!strcmp(thermotype, "Andersen")){
			...
			if((currentSteps%200)==0) {
	    			vtPlus[i].x = gaussVel(qq);
	    			vtPlus[i].y = gaussVel(qq);
	    			vtPlus[i].z = gaussVel(qq);
...
//cout << " molNDF =  "  << molNDF << "   atomNDF  = "<< atomNDF <<endl;
	...
	if (!strcmp(thermotype, "NoseHoover")){
		...
		//if (currentSteps <= 0.5*totsteps)
		{
			if(useAtomThermo){
			nht = nht + 1;
			fnh = fnh + timeStep*invqq*(atomKinEnergy - atomNDF*BOLTZMAN*temperature);
		}
			else{
			fnh = fnh + timeStep*invqq*(molKinEnergy - molNDF*BOLTZMAN*temperature);
			nht = nht + 1;
			}
		}
		...
		*ofNH << fixed << setprecision(6) << setw(14) << qq << endl;
		}
	}
	else { fnh = 0.0;}
...
// Jc: calculate the Heat Capacity designed by Jianhui LI on 31 Mar 2006
        if((currentSteps>0 )&&(prank ==0)){
          ...
          double aTSumAverage2 = (aTSum/currentSteps)*(aTSum/currentSteps);
          double aT2SumAverage = aT2Sum/currentSteps;
          double mTSumAverage2 = (mTSum/currentSteps)*(mTSum/currentSteps);
          double mT2SumAverage = mT2Sum/currentSteps;
          double E2Average = E2Sum/(currentSteps);
          double ESumAverage2 = (ESum/(currentSteps))*(ESum/(currentSteps));
          double Cv = 120371.7305*(E2Average - ESumAverage2)/(myEnsemble->temperature*myEnsemble->temperature)/numMols;
          double aCv = 120371.7305*(E2Average - ESumAverage2)/(aTSumAverage2*500);
          double mCv = 120371.7305*(E2Average - ESumAverage2)/(mTSumAverage2*500);
          double aCV = 8.3145/(0.6666667-500*(aT2SumAverage-aTSumAverage2)/(aTSumAverage2));
          double mCV = 8.3145/(0.6666667-500*(mT2SumAverage-mTSumAverage2)/(mTSumAverage2));
// Jc: Heat capacity unit is [KJ/(Kg*K)], details see Jianhui Notebook on 1st April 2006
        if(((currentSteps%100)==0)&&(prank==0))
        {
            ...
            *ofPotential<<setw(9)<<myEnsemble->myEnergy.ljEnergy/numMols;
            *ofPotential<<setw(9)<<myEnsemble->myEnergy.realEnergy/numMols;
            *ofPotential<<setw(7)<<myEnsemble->myEnergy.longEnergy/numMols;
            *ofPotential<<setw(9)<<myEnsemble->myEnergy.molCorrectEnergy/numMols;
            *ofPotential<<setw(6)<<myEnsemble->myEnergy.surfCorrectEnergy/numMols;
            *ofPotential<<setw(9)<<myEnsemble->myEnergy.correctEnergy/numMols;
            *ofPotential<<setw(7)<<myEnsemble->myEnergy.inducEnergy/numMols;
            *ofPotential<<setw(6)<<myEnsemble->myEnergy.TBEnergy/numMols;                              
            *ofPotential<<setw(8)<<myEnsemble->myEnergy.totEnergy/numMols;
            *ofPotential<<setprecision(4)<<setw(8)<<" Cv="<<Cv<<" aCv="<<aCv<<" mCv="<<mCv<<" aCV="<<aCV<<" mCV="<<mCV<<" Steps="<<currentSteps<<" aT="<<atomTemp<<" mT="<<molTemp<<" <E2>-<E>2="<<E2Average-ESumAverage2<<endl;
        }
        }
...
vomz = vomz/masstott;
    ...
    for (Int i = 0; i < numAtoms; i++)
    {
        myAtoms[i].velocity.x = factor*(myAtoms[i].velocity.x - vomx) + vomx;
        myAtoms[i].velocity.y = factor*(myAtoms[i].velocity.y - vomy) + vomy;
        myAtoms[i].velocity.z = factor*(myAtoms[i].velocity.z - vomz) + vomz;
        vtPlus[i].x = factor*(vtPlus[i].x - vomx) + vomx;
        vtPlus[i].y = factor*(vtPlus[i].y - vomy) + vomy;
        vtPlus[i].z = factor*(vtPlus[i].z - vomz) + vomz;
        vtMinus[i].x = factor*(vtMinus[i].x - vomx) + vomx;
        vtMinus[i].y = factor*(vtMinus[i].y - vomy) + vomy;
        vtMinus[i].z = factor*(vtMinus[i].z - vomz) + vomz;
    }
...
//       cout<<"  id0 =  "<< id0 << "   id1 =  "<< id1 << "  mass0 = "<< massO << "  massC = "<< massC <<endl;
            ...
            rOld[nm][1] = atom1->position;
            vtPlus[id1] = vtMinus[id1] + timeStep*mass_r[id1]*(atom1->force - (massC/MM)*0.5*atom1->force + (massO/MM)*atomc->force - (massC/MM)*0.5*atom->force);
            rNew[nm][1] = rOld[nm][1] + timeStep*vtPlus[id1];
            //moving[nm][1] = false;
            //moved[nm][1]  = true;

            rOld[nm][2] = atomc->position;
...
//cout<<"  diffco1   =  "<< diffco1 << "  diffco2 =  "<< diffco2 << endl;
        ...
        {   
	 	    ...
	 	    myEnsemble->apply_pbc(rijNew); 
                    ...
                    diff = bondLength[nm][0] - rijNew2;         // JC the difference between 
	 	        ...
	 	        myEnsemble->apply_pbc(rijOld); 
                        ...
                        if (rijNewOld < bondLength[nm][0]*tol)
                        {
...
//     {
    ...
    {                                        // created by ZhongWu
        ...
        // we only treat water as cyclic molecule here and water must have a name of "WAT"
        if (strcmp(mol->molName, "WAT"))     // not water 
           {
            nBonds = ns - 1; 
           }
        else
        ...
        // update atomic positions
//cout<<"  MolMass = "<<mol->mass <<"  MolType = "<<mol->molType<<"  MolID = "<<mol->molID<<"  ns = "<<ns<<"  nBonds = "<<nBonds<<endl;
        for (Int i = 0; i < ns; i++) // calculate the new assumed positions of atoms on nmth molecule
        {
            Atom *atom = mol->myAtoms[i];
            Int id = atom->atomID;
          // cout<<"  i =  "<< i << "   ID =  "<< id << "mass_r[id]  =  "<< mass_r[id] << endl;
            rOld[nm][i] = atom->position;
...
//    MPI::COMM_WORLD.Bcast(posTemp,4500,MPI::DOUBLE,0);
    ...
    MPI_Bcast(posTemp,4500,MPI_DOUBLE,0,MPI_COMM_WORLD);
