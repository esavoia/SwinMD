	...
	double erf(double x) 
{
	...
	if(ix < 0x3feb0000) {		/* |x|<0.84375 */
	    if(ix < 0x3e300000) { 	/* |x|<2**-28 */
	        if (ix < 0x00800000) 
		    return (double) 0.125*(8.0*x+efx8*x);  /*avoid underflow */
		return x + efx*x;
	    }
	    z = x*x;
	...
	if(ix < 0x3ff40000) {		/* 0.84375 <= |x| < 1.25 */
	    ...
	    if(hx>=0) return (double) erx + P/Q; else return (double) -erx - P/Q;
	}
	if (ix >= 0x40180000) {		/* inf>|x|>=6 */
	    if(hx>=0) return (double)one-tiny; else return (double)tiny-one;
	}
	x = fabs(x);
 	s = one/(x*x);
	...
	if(hx>=0) return (double) one-r/x; else return (double) r/x-one;
}
